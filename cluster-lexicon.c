...
int load_lex(char* file_name, int* lexicon, int lex_size)
{
    ...
    if (NULL == (fp = fopen(file_name, "r")) || UNDEF == fseek(fp, 0, SEEK_END) || UNDEF == (len = ftell(fp)))
...
int main(int argc, char* argv[])
{
    ...
    float threshold, dist, min_dist, d1, d2,dis;
    FILE *cfp, *model;
    ...
    {
        if (argc != 6) 
            ERR_MESG("Usage: a.out train lexicon lexicon-size model-file threshold");
        if (0 >= (lex_size = atoi(argv[3])))
        ...
        if (NULL == (cfp = fopen(argv[4], "w"))) 
            ERR_MESG("error opening cluster / stem file");
        if (NULL == (lexicon = Malloc(lex_size, int)))
            ERR_MESG("lexicon - out of memory");
        if (UNDEF == (lex_size = load_lex(argv[2], lexicon, lex_size)))
            return UNDEF;


        max_distances = (unsigned) lex_size * (unsigned) lex_size / 50;
        if (NULL == (c_distances = Malloc(max_distances, DISTANCE)) || NULL == (clusters = Malloc(lex_size, CLUSTER)))
    ...
    while(1) {
        ...
        for (i = clusters[c1].start_offset; i < clusters[c1+1].start_offset; i++) {
            ...
            if (d1 > c_distances[i].dist) 
                c_distances[i].dist = d1;
        }
